...
class HookEntry : IYukiHookXposedInit {
                ...
                DexKitBridge.create(apkPath).use { bridge ->
                        ...
                        injectMember {
                            ...
                            afterHook {
                                try {
                                    ...
                                    val statusBarView = readField(controller, "mView") as? ViewGroup ?: return@afterHook
                    ...
                    try {
                        ...
                        if (colorUpdateMethodData == null) {
                            ...
                            loggerD(msg = "BatteryTemp: NetworkSpeedView color update method = ${colorUpdateMethod.toGenericString()}")
                            findClass(networkSpeedViewClass.name).hook {
                                injectMember {
                                    method {
                                        name = colorUpdateMethod.name
                                        paramCount(colorUpdateMethod.parameterTypes.size)
                                    }
                                    afterHook {
                                        try {
    ...
    companion object {
        ...
        @Volatile private var injectedTextView: WeakReference<TextView>? = null
        ...
        private fun startTemperatureUpdater(target: TextView) {
            ...
            val receiver = object : BroadcastReceiver() {
                override fun onReceive(context: Context, intent: Intent) {
                    if (intent.action == Intent.ACTION_BATTERY_CHANGED) batteryIntent[0] = intent
                }
            }
            ...
            if (target.isAttachedToWindow) start()
        }
        ...
        private fun findLeftSideGroup(root: ViewGroup): ViewGroup? {
            ...
            val resourceNames = arrayOf("phone_status_bar_left_container", "status_bar_left_container", "status_bar_left")
            ...
            return findViewGroup(root) { view ->
                ...
                role.contains("left") && (role.contains("container") || role.contains("statusbar") || role.contains("status_bar"))
            }
        }
        private fun findClockView(root: ViewGroup): TextView? {
            val context = root.context
            for (name in arrayOf("clock", "status_bar_clock")) {
                val id = context.resources.getIdentifier(name, "id", "com.android.systemui")
                if (id != 0) {
                    val view = root.findViewById<View>(id)
            ...
            return findTextView(root) { view ->
                ...
                if (classNameCandidate == null) {
                    ...
                    if (simpleName == "Clock" || simpleName.endsWith("Clock")) classNameCandidate = view
                }
                false
            } ?: classNameCandidate
        }
        ...
        private fun View.resourceEntryName(context: Context): String = try {
